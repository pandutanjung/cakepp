...
<html lang="en">
  ...
  <body>
    ...
    <nav class="navbar bg-body-tertiary bg-info-subtle fixed-top">
      ...
      <!-- Week Card -->
    <div class="row row-cols-1 row-cols-md-4 g-3 mt-2 ms-2 me-2">
              <?php
                include("config.php");
                $username = $_SESSION['username'];
                $query = pg_query($db, "SELECT * FROM pertemuan WHERE akun_username = '$username'");
                while ($pertemuan = pg_fetch_assoc($query)) {
              ?>
        <div class="col">
          <div class="card text-center" style="width: 18rem;">
            ...
            <h5 class="card-title">Week <?=$pertemuan['pertemuan_ke']?></h5>
                    ...
                    <a href="lihatweek.php?id=<?= $pertemuan["id"]?>" class="btn btn-info text-light"><i class="bi bi-folder2-open"></i> Show</a>
                    <a href="hapusweek.php?id=<?= $pertemuan["id"]?>" class="btn btn-danger text-light"><i class="bi bi-folder-x"></i> Delete</a>
            </div>
          </div>
        </div>
        <?php } ?>
